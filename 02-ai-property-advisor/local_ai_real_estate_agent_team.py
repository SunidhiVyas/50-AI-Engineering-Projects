...
class DirectFirecrawlAgent:
    ...
    def __init__(self, firecrawl_api_key: str, model_id: str = "qwen2.5:0.5b"):
        self.agent = Agent(
...
1. Find up to 5 property listings on the page
...
3. CRITICAL REQUIREMENTS:
   - Extract up to 5 relevant properties
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("🔑 API Keys", expanded=True):
            ...
            # Ollama model info
            st.info("🤖 Using Ollama model: qwen2.5:0.5b (local)")
            st.markdown("Make sure Ollama is running with: `ollama run qwen2.5:0.5b`")
        ...
        with st.expander("🌐 Search Sources", expanded=True):
            ...
            selected_websites = [site for site in available_websites if st.checkbox(site, value=site == "Zillow")]
